export function countryCodeToFlag(countrycode:string) {
    ...
    const A = 65;
    const Z = 90;
    const f = countrycode.codePointAt(0) || 127487;
    const s = countrycode.codePointAt(1) || 127487;
  ...
  export function flagToCountryCode(flag:string) {
    if (flag === '🌍') return 'xx' // not a country, used for «all»
  
    const offset = 127397;
    const f = flag?.codePointAt(0) || 127487;
    const s = flag?.codePointAt(2) || 127487;
...
export const getCountryName = (countryCode:string) => Object.getOwnPropertyDescriptor(isoCountries, countryCode) ? isoCountries[countryCode] : countryCode